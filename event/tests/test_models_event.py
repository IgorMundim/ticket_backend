...
from event.models import Batch, Leasing
...
class EventModelTeste(EventTestBase, AccountTestBase):
    def setUp(self) -> None:
        ...
        self.category = self.make_category(slug="")
        self.account = self.make_account_create_user(
            email="event@user.com",
        ...
        self.event_two = self.make_event(
            account=self.account
        )
    ...
    def test_create_single_addrees(self):
        self.assertEqual(
            str(self.address),
            ("%s - %s" % (self.address.city, self.address.uf))
        )
    ...
    def test_create_single_batch(self):
        ...
        self.assertEqual(Batch.objects.is_valid_change(
            id=0,
            sales_qtd=2,
            batch_stop_date=datetime.strptime(
                '28 11 2022 09:59:34 +0000 (UTC)',
                "%d %m %Y %H:%M:%S %z (%Z)"
            ), event_pk=1,
        ), False)
        self.assertEqual(Batch.objects.is_valid_change(
    ...
    def test_create_single_leasing(self):
        ...
        self.assertIsNotNone(Leasing.objects.get_leasing(event_pk=1))
        self.assertIsNotNone(Leasing.objects.get_leasing(event_pk=2))
